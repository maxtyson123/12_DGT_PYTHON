...
def delete_portion(file_path, start_line, end_line):
    ...
    for number, line in enumerate(lines):  # convert lines to number
        if number not in range(start_line, end_line):  # if its not in the selected range then print as normal
            file.write(line)
        else:
...
def read_portion(file_path, start_line, end_line):
    ...
    for number, line in enumerate(lines):  # convert lines to number
        try:
            if number in range(start_line, end_line):  # if its not in the selected range then print as normal
                print(line)
        except:
            return
    file.close()  # close the file
...
def get_end_line(file_path, start_line, end_indicator):
    ...
    for x in lines:
        current += 1
        if int(current) == int(start_line):  # if the current line is same as the one specified
            current2 = current  # update second counter to the started
            begin_counter_2 = True  # set that the start line has been found
        elif (begin_counter_2 is True):  # this is so that it will find the relvent end tag rathern then first one it finds
...
def add_to_order(stritem_id, amt):
    ...
    if customer_data[0] == "DEFAULT":  # check if there is the required info
        if bot_run != True:
            ...
            customer_details("run")  # this is becuase it needs to know if the order is cooked or frozen, so it can apply the discount
        else:
            return "Customer Info not entered"
    if bot_run != True:
        amt_of_fish = check_fish(food_list[item_id], user_order[0])
        if (item_id != chip_id):  # if the program isnt adding chips, show tha max amount of fish and how many the user has ordered
            print(
                " You have ordered: "
                + str(amt_of_fish)
                + " "
        ...
        amt = input(colour + " How many " + food_list[item_id] + " would you like: " + "\x1b[0m")  # ask how many of the item the user would like
    try:
        int(amt)  # trys to convert to int
        is_int = True
    except ValueError:  # if it cant the return error
        is_int = False
    if is_int is False:  # if it is a error
        if print_data == "data":
    ...
    if item_id == chip_id:  # IF ITS chips
        print(amt)
        chips_price = price_list[item_id] * float(amt)  # convert to int times the price by the amt
        print(chips_price)
        if print_data == "data":
            print("set chips price")
        total_cost = total_cost + chips_price  # add the cost
        user_order[0].append(str(amt + " scoops of chips"))  # and the amount of scoops
        user_order[1].append(chips_price)  # add the cost to the list
        if print_data == "data":
            print("appened to list")
        if (bot_run != True):  # this just makes sure that an external program doesnt run this
            fish_menu()  # go back
    for x in range(int(amt)):
        ...
        if amt_of_fish == 7:
            ...
            print("Added " + food_list[item_id] + ": " + str(x + 1))  # this is to show the user that the item actually has been added,  add one to make it count properly bc lists idex starting at 0

            total_cost = total_cost + price_list[item_id]  # add to the cost
            if print_data == "data":
                print("added to vars")
            fish_price = price_list[item_id]
            if customer_data[2] == "0":
                if print_data == "data":
                    print("Fish type: Frozen")
                total_cost = total_cost - 1.05  # take away the discount
                fish_price = price_list[item_id] - 1.05
                user_order[0].append(food_list[item_id])
            else:
                user_order[0].append(food_list[item_id])
            user_order[1].append(fish_price)
            if print_data == "data":
                print("appened to list")
...
def set_history():
    ...
    history_doc.write("----------------")  # Header the doc (used to indecate a break between sessions in the history)
    history_doc.close()
...
def reset():
    ...
    if (
        ...
        f.close()
    debugTag = debug_check("data")  # Checks if color has been set in debug mode
...
def print_dual_menu(multi_array):
    ...
    for x in range(
        ...
        price_tag = ("#" + "$" + price.ljust(4)[:4] + "#")  # this is to make the whole output is formated with ljust
        numbering = ("[" + str(x) + "] ")  # if it wasnt like this then I would have to calculate and add a ljust for everything in the print function
        print(numbering.ljust(8)[:8] + print_this[x].ljust(99)[:99] + price_tag.ljust(8)[:8])
    print(devider)
...
def logo():
    ...
    debugTag = debug_check("Color")  # Checks if color has been set in debug mode
...
def venv():
    ...
    if venv_check == "venv":
        ...
        print("Made files")
        # CHECKING THE VENV
        venv_data = check_file("data.txt", "venvMadeTrue") 
        if venv_data is True:  # if the user has made the virtual enviroment before
            if print_data == "data":
                print("VENV DATA TRUE")
            set_custom_data = check_file("data.txt", "CustomEnviroment")
            if set_custom_data is True:
            ...
            requirmenets = [
                "Keyboard"
            ]  # Yes I know that there is only one in the list but that is to make this script more scalable and future proof.
        ...
        if process_started is True:  # check if a cmd process with venv has started
            f = open("venv.txt", "r+")
            f.truncate(0)  # reset it
            if debugTag == "data":
                print("Reset file")
...
def remove_item():
    ...
    if (back_id == 0):  # if there is nothing in the user order array then there is nothing to remove
        main()  # go back to the menu
    for x in user_order[0]:
        order.append(x)  # add everything from the order to the duplicate array
    order.append("Type 'back' To Go Back")  # add back text to duplicate array, this way "back" text is not added to their order
    user = 999
    logo()  # Print the logo
    ...
    for x in back:
        user = (user.lower())  # make the input lowercase, this makes less combinations of "back"
        if user == x:
    ...
    if is_int != False:  # if it is not a error
        ...
        if int(user) in range(orderRange):
            ...
            total_cost -= user_order[1][int(user)]  # Take away the price from the total cost
...
def run_again():
    ...
    clear()
    user = 999
    allowdebugTag = debug_check("debug")
    logo()  # Print the logo
...
def finish():
    ...
    user = 999
    logo()  # Print the logo
    if (customer_data[0] == "DEFAULT" or len(user_order[0]) == 0):  # check if there is the required info
    ...
    print("Price: $" + "{:.2f}".format(total_cost))  # format the cost becuase python does maths wrong with memory errors so it  will look like 41.0000000000006 instead og 41.00
    print(devider )
    set_history()
    f = open("history.txt", "a")
    f.write("Finished" + "\n")
...
def customer_details(mode):
    ...
    if mode == "run":
        ...
        name = input(colour + "Please enter your name: " + "\x1b[0m")  # Name allows numbers becuase of people who may be named like Prince Harray the 3rd
        if name == "DEFAULT":
        ...
        phoneNumber = input(colour + "Please enter your phone number: " + "\x1b[0m")  # get the number
        ...
        try:
            ...
            phoneisNumber = False
        if phoneisNumber is False:
            error("'" + str(phoneNumber) + "' is not a number")
            customer_details(1)  # re-run
        customer_data[1] = ("+64 " + phoneNumber)  # store number in slot, add the +64 as the int checker doesnt allow that
        return
    elif mode == 2:
        ...
        if customer_data[3] == "1":
            customer_data[4] = "N/A"  # if it is not then set the adress to the default, this is for when changin from delivery to pickup
        return
    elif mode == (4):
...
def main():
    ...
    if len(ordercheck) != 0:
        ...
        remv = len(main_print_single_menu) - 1  # get its place for the input
    main_print_single_menu.append("Exit")
    exit1 = len(main_print_single_menu) - 1  # get its place for the input
    print_single_menu(main_print_single_menu)  # Print the menu
    user = input(
        colour
        + " Please make a choice via number and then press enter to confirm: "
        + "\x1b[0m"
    ...
    if user == "0":
        ...
        if ignore_history != "ignore_history":
            ...
            f.close()
        quit()
    elif user == "debug" and allowdebugTag == "debug":
        print("debug MODE")
        debug()
    else:
        error("'" + user + "' is not a option")
...
def debug():
    ...
    venv_check = debug_check("venv")  # Check if virtual
    if venv_check == "venv":
        debug_print_single_menu.append("Install a python package")  # Add venv required options
        venvOp1 = len(debug_print_single_menu) - 1  # check how many in array
        debug_print_single_menu.append("Execute a python file in a venv")
        venvOp2 = len(debug_print_single_menu) - 1 # check how many in array
    ...
    exit1 = len(debug_print_single_menu) - 1  # get its place for the input
...
def exit_handler():
    print("Thank you for eating at Freddy's Fast Fish")
    time.sleep(1)
...
if bot_run != True:
    ...
    debugTag = debug_check("data")  # Checks if data has been set in debug mode
    ignore_history = debug_check("ignore_history")  # Checks if ignore_history has been set in debug mode, this is just so Im not creating loads of entrys into the file while testing
    reset()
    if ignore_history != "ignore_history":
        f = open("history.txt", "a")
    ...
    idledebugTag = debug_check("idle")  # SAME HERE
    if debugTag == "data":
        print("Checked for idle and for idle debug tag")
        print("idleCheck: " + str(idleCheck))
    venv()
    if idleCheck is True:  # IF it is then run the next check
        if (idledebugTag == "idle"):  # if the debug tag is set to idle then alert that it is and continue
            if debugTag == "data":
