...
def deletePortion(FilePath, startLine, endLine):
     ...
     for number, line in enumerate(lines): #convert lines to number
        if number not in range(startLine, endLine): #if its not in the selected range then print as normal
            file.write(line)
        else:
             file.write("Data DELETED \n") #if is then write data deleted
     file.close()       #close the file
def readPortion(FilePath, startLine, endLine):     
     file = open(FilePath, 'r+') #open the file (read)
     lines = file.readlines()    #store as lines 
     for number, line in enumerate(lines): #convert lines to number
       try:   
        if number in range(startLine, endLine): #if its not in the selected range then print as normal
         print(line)
       except:
          return  
     file.close()       #close the file     
def getEndLine(FilePath, sartLine, endIndicator):
     ...
     lines = file.readlines() #read the lines
     beginCounter2 = False #counter state
     for x in lines:      
         current += 1
         if int(current) == int(sartLine): #if the current line is same as the one specified
            current2 = current             #update second counter to the started   
            beginCounter2 = True           #set that the start line has been found       
         elif beginCounter2 == True:       #this is so that it will find the relvent end tag rathern then first one it finds  
             current2 += 1                 #star coutning   
             if endIndicator in x:         #now if it finds the end tag       
...
def checkTwoLines(FilePath, fileArgs1, fileArgs2):
     ...
     for x in lines:
         current += 1
         if fileArgs1 in x and fileArgs2 in x: #if both strings are on one line 

             return current #return counter
     return False

def fakeDetails():            #used to skip menus when debug
 global customerData
 customerData.append("ORDER VIA BOT") #fake name
 customerData.append("123456789")       #fake phone numer  
 customerData.append(1)               #cooked   
 customerData.append(1)               #pick up  
def fishNameToNumber(fishName): #not used, but was going to be
     global fishs
     fish = fishs[0]
     fishNumber = 0
     for x in fish:  #for everything
          fishNumber += 1   #count + 1
          if fishName == x:
             return fishNumber #return fish number
     return("Not a fish")     
...
def howManyInArray(arrayName):
     arrayNum = 0
     for x in arrayName: #for every thing in the array
         arrayNum += 1 #counter + 1
     return arrayNum   
...
def addToOrder(strItemID, amt):
     ...
     chipCounter = 0   #makes the menu more flexible, can now add fish items to any spot in the array
     for x in foodList:
         chipCounter += 1   
         if x == "Scoop Of Chips":
            chipCounter -= 1  
...
def reset():
     ...
     printData = debugCheck("data") #Checks if color has been set in debug mode
     if debug == "data":
...
def printDualMenu(multiArray):
    ...
    print ("##################################################################################################################")
    for x in range(len(printThis)): #for everything in the array print it plus its number
        price = str("{:.2f}".format(priceThis[x]))
        priceTag = "#"+"$"+price.ljust(4)[:4]+"#" #this is to make the whole output is formated with ljust
        ...
        print (numbering.ljust(8)[:8]+printThis[x].ljust(99)[:99]+priceTag.ljust(8)[:8])
...
def runInIdle():
 ...
 if user == "0":
    ...
    error("IF YOU RUN IN IDLE, PLEASE EXIT USING THE OPTIONS ON THE SCRIPT, OTHERWISE HISTORY CAN BE MESSED UP")
    main()
 elif user == "2":
      print("Exiting")
      if ignoreHistory != "ignoreHistory":
...
def DEBUG():
 ...
 if venvCheck == "venv":
   ...
   venvOp1 = howManyInArray(debug_printSingleMenu)  #check how many in array
   venvOp1 = venvOp1 - 1
   debug_printSingleMenu.append("Execute a python file in a venv")
   venvOp2 = howManyInArray(debug_printSingleMenu)  #check how many in array
   venvOp2 = venvOp2 - 1
 chatBotCheck = debugCheck("chatBot")  
 if chatBotCheck == "chatBot":
  
   debug_printSingleMenu.append("Run the bot")#Add venv required options
   chatBotOp1 = howManyInArray(debug_printSingleMenu)  #check how many in array
   chatBotOp1 = chatBotOp1 - 1
   debug_printSingleMenu.append("Train the bot")
   chatBotOp2 = howManyInArray(debug_printSingleMenu)  #check how many in array
   chatBotOp2 = chatBotOp2 - 1
   debug_printSingleMenu.append("Add bot data")
   chatBotOp3 = howManyInArray(debug_printSingleMenu)  #check how many in array
   chatBotOp3 = chatBotOp3 - 1
   debug_printSingleMenu.append("Install required packages")
   chatBotOp4 = howManyInArray(debug_printSingleMenu)  #check how many in array
 ...
 if user == "0":
    ...
    DEBUG()
     ...
     DEBUG()
      ...
      debugMode.append("data") #append data to debug tags
      DEBUG()
 elif user == "3":
     ...
     if startLine == False:
          ...
          DEBUG()
     startLine -= 1     
     endLine = getEndLine("history.txt", startLine, "----------------")
     if endLine == False:
